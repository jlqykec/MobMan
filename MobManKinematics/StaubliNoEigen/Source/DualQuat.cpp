...
{
	return DualQuat(Quat(cos(angle*0.5), sin(angle*0.5)*d), Quat(0, sin(angle*0.5)*m));
}
...
Eigen::Vector3d DualQuat::linesIntPlucker(DualQuat l1, DualQuat l2)
{
	...
	return l1.getPrim().getV().cross(l1.getDual().getV()) + l2.getPrim().getV().cross(l2.getDual().getV()).dot(l1.getPrim().getV())*l1.getPrim().getV();
}
